...
def go2(mqtt_sender, x, speed):
    print()
    print("Left and right wheel motor speed:", speed)
    mqtt_sender.send_message("spin_until_facing", [x, speed])
...
def handle_spin_until_facing(x_box, spin_left_box, mqtt_sender):
    x = int(x_box.get())
    speed = int(spin_left_box.get())
    go2(mqtt_sender, x, speed)
